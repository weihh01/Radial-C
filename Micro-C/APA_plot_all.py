...
clr_dmso = cooler.Cooler(data_dir + '/DMSO_20min.cool')
clr_tpa = cooler.Cooler(data_dir + '/TPA_20min.cool')
...
loop_prefix_list = ["outwardstatic","staticstatic","staticinward"]

for loop_prefix in loop_prefix_list:
	file_name = f'CC_{loop_prefix}.bedpe'
	#column_names = ['chrom1', 'start1', 'end1', 'chrom2', 'start2', 'end2', 'name', "strand"]
	column_names = ['chrom1', 'start1', 'end1', 'chrom2', 'start2', 'end2', "strand"]
	...
	globals()[f'CC_{loop_prefix}'] = paired_sites
...
for loop_prefix in loop_prefix_list:
	for condition in ["dmso", "tpa"]:
		stack = cooltools.pileup(eval(f'clr_{condition}'), eval(f'CC_{loop_prefix}'), view_df=hg38_arms,
					expected_df=eval(f'expected_{condition}'), flank=100_000)
		sites = eval(f'CC_{loop_prefix}')
...
for loop_prefix in loop_prefix_list:
	for condition in ["dmso", "tpa"]:
		...
		np.savetxt(f'APA_plot_mtx_{condition}_{loop_prefix}.tsv', eval(f'mtx_{condition}_{loop_prefix}_p'),
			delimiter='\t')
	globals()[f'mtx_fc_{loop_prefix}_p'] = eval(f'mtx_tpa_{loop_prefix}_p') / eval(f'mtx_dmso_{loop_prefix}_p')
	np.savetxt(f'APA_plot_mtx_fc_{loop_prefix}.tsv', eval(f'mtx_fc_{loop_prefix}_p'), delimiter='\t')
...
loop_prefixes = ["outwardstatic","staticstatic","staticinward"]
...
plt.savefig('CTCF.pdf', format="pdf", bbox_inches='tight')