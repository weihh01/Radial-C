...
loop_prefix_list = ["none","inwardoutward"]

for loop_prefix in loop_prefix_list:
	file_name = f'{loop_prefix}.bedpe'
	...
	globals()[f'{loop_prefix}'] = paired_sites
...
for loop_prefix in loop_prefix_list:
	for condition in ["dmso", "tpa"]:
		stack = cooltools.pileup(eval(f'clr_{condition}'), eval(f'{loop_prefix}'), view_df=hg38_arms,
					expected_df=eval(f'expected_{condition}'), flank=100_000)
		sites = eval(f'{loop_prefix}')
...
loop_prefixes = ["none","inwardoutward"]
