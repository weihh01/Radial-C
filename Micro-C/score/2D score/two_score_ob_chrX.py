...
fr = open("/mnt/disk4/whh/score/bin/100kb/chrX.bed")
...
fr = open("MNase-1min_OE_chrX.bed")
...
fr = open("MNase-5min_OE_chrX.bed")
...
fr = open("MNase-20min_OE_chrX.bed")
...
np.savetxt("col_ave_OE_chrX.txt",col_ave,fmt='%.6f')
np.savetxt("returnMat_ob_chrX.txt",returnMat_all,fmt='%.6f')
col_ave = np.sum(Mat_30min, axis=0)/np.sum(Mat_5min, axis=0) + np.sum(Mat_5min, axis=0)/np.sum(Mat_1min, axis=0)
np.savetxt("col_ave_OE_chrX_new.txt",col_ave,fmt='%.6f')
